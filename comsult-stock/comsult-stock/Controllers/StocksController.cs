...
namespace consult_stock.Controllers
{
    ...
    public class StocksController : ControllerBase
    {
        ...
        {
            var articles = await _stockService.GetAvailableArticlesAsync();
            var lots = await _stockService.GetAvailableLotsAsync();
            var societes = await _stockService.GetAvailableSocietesAsync();
            
            return Ok(new { 
                articles = articles.Select(a => new { a.Id, a.Nom, a.CodeArticle }),
                lots = lots.Select(l => new { l.Id, l.NumLot, l.ArticleId }),
                societes = societes.Select(s => new { s.Id, s.Nom })
            });
        }
        ...
        public async Task<IActionResult> Create([FromBody] CreateStockDto dto)
        {
            try
            {
                var stock = new Stock
                {
                    ...
                    // PrixTTC sera calculé automatiquement dans le service
                };
