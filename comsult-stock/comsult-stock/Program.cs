...
builder.Services.Configure<SmtpSettings>(builder.Configuration.GetSection("Smtp")); // <-- ici
...
using (var scope = app.Services.CreateScope())
{
    ...
    catch (Exception ex)
    {
        ...
        // En cas d'erreur, essayer de créer au moins la structure de base
        try
        {
            await context.Database.EnsureCreatedAsync();
        ...
        {
            Console.WriteLine($"? Erreur critique lors de la création de la base: {dbEx.Message}");
        }
    }
}
