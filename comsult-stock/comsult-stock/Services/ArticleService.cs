...
namespace consult_stock.Services
{
    public class ArticleService
    {
        ...
        public async Task<IEnumerable<Article>> GetBySocieteIdAsync(int societeId)
        {
            ...
            var articleIds = await _context.Stocks
                .Where(s => s.SocieteId == societeId && s.ArticleId.HasValue)
        ...
        public async Task<IEnumerable<ArticleSelectDto>> GetArticlesForSelectAsync()
        {
            return await _context.Articles
                .Select(a => new ArticleSelectDto
                {
                    Id = a.Id,
                    Nom = a.Nom,
        ...
        public async Task<IEnumerable<ArticleSelectDto>> GetArticlesWithStockBySocieteAsync(int societeId)
        {
            return await _context.Articles
                .Where(a => a.Stocks.Any(s => s.SocieteId == societeId && s.QteDispo > 0))
                .Select(a => new ArticleSelectDto
                {
                    Id = a.Id,
                    Nom = a.Nom,
