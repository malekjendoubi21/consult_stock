...
namespace consult_stock.Services
{
    public class StockService
    {
        ...
        public async Task<Stock> CreateAsync(Stock stock)
        {
            ...
            Console.WriteLine($"Creating stock - SocieteId: {stock.SocieteId}, ArticleId: {stock.ArticleId}, LotId: {stock.LotId}");
            ...
            if (stock.ArticleId.HasValue)
            {
                ...
                if (article == null)
                {
                    // List all available articles for debugging
                    var availableArticles = await _context.Articles.Select(a => a.Id).ToListAsync();
                    throw new ArgumentException($"Article avec l'ID {stock.ArticleId.Value} introuvable. Articles disponibles: [{string.Join(", ", availableArticles)}]");
                }
            ...
            if (stock.LotId.HasValue)
            {
                ...
                if (lot == null)
                {
                    // List all available lots for debugging
                    var availableLots = await _context.Lots.Select(l => l.Id).ToListAsync();
                    throw new ArgumentException($"Lot avec l'ID {stock.LotId.Value} introuvable. Lots disponibles: [{string.Join(", ", availableLots)}]");
                }
                ...
                if (stock.QteDispo > lot.QuantiteDisponible)
                    throw new ArgumentException($"Quantité demandée ({stock.QteDispo}) supérieure à la quantité disponible du lot ({lot.QuantiteDisponible}).");
            }
        ...
        public async Task<IEnumerable<Article>> GetAvailableArticlesAsync()
        {
            return await _context.Articles.ToListAsync();
        }

        public async Task<IEnumerable<Lot>> GetAvailableLotsAsync()
        {
            return await _context.Lots.ToListAsync();
        }

        public async Task<IEnumerable<Societe>> GetAvailableSocietesAsync()
        {
