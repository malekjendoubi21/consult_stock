...
namespace consult_stock.Services
{
    public class VenteService
    {
        ...
        public async Task<VenteCreatedResponseDto> CreateVenteAvecCalculAsync(VenteArticleDto dto)
        {
            ...
            // 3. Récupérer l'article par son code
            ...
            if (lot.QuantiteDisponible < dto.Quantite)
                throw new ArgumentException($"Quantité insuffisante. Disponible: {lot.QuantiteDisponible}, Demandée: {dto.Quantite}");
            ...
            // 8. Mettre à jour la quantité disponible du lot
            lot.QuantiteDisponible -= dto.Quantite;

            // 9. Mettre à jour le stock si il existe
            ...
            // 11. Créer des tickets si demandé
            ...
            // 12. Retourner la réponse complète
        ...
        public async Task<VenteCreatedResponseDto?> UpdateVenteAvecCalculAsync(int id, UpdateVenteArticleDto dto)
        {
            ...
            // 2. Récupérer la vente existante
            ...
            // 6. Calculer la différence de quantité
            var differenteQuantite = dto.Quantite - existing.QteVendu;
            
            // 7. Vérifier la disponibilité si augmentation
            if (differenteQuantite > 0 && lot.QuantiteDisponible < differenteQuantite)
                throw new ArgumentException($"Quantité insuffisante pour l'augmentation. Disponible: {lot.QuantiteDisponible}, Demandée en plus: {differenteQuantite}");

            // 8. Calculer les nouveaux prix
            dto.CalculerPrix(lot.PrixUnitaire, dto.Quantite);

            // 9. Mettre à jour la vente
            ...
            {
                existing.LotEntity.QuantiteDisponible += existing.QteVendu; // Remettre l'ancienne quantité
            }
            lot.QuantiteDisponible -= dto.Quantite; // Retirer la nouvelle quantité
