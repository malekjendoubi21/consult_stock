...
export class UsersComponent implements OnInit {
  ...
  // --- VENDEUR CRUD ---
  loadVendeurs() {
    this.vendeurService.getAllBackoffice().subscribe({
  ...
  addVendeur() {
    // Utiliser CreateVendeurDto avec les bons champs
    const createDto = {
      nom: this.newVendeur.nom,
      email: this.newVendeur.email,
      password: this.newVendeur.passwordHash || '' // Le DTO utilise 'password' et non 'passwordHash'
    };
    this.vendeurService.createBackoffice(createDto).subscribe({
      next: () => {
        this.loadVendeurs();
  ...
  updateVendeur() {
    ...
    // Utiliser UpdateVendeurDto avec les bons champs
    const updateDto = {
      nom: this.editingVendeur.nom,
      email: this.editingVendeur.email,
      password: this.editingVendeur.passwordHash // Optionnel pour la mise à jour
    };
    this.vendeurService.updateBackoffice(this.editingVendeur.id, updateDto).subscribe({
      next: () => {
        this.loadVendeurs();
  ...
  deleteVendeur(id: number) {
    if (confirm('Voulez-vous supprimer ce vendeur ?')) {
      this.vendeurService.deleteBackoffice(id).subscribe(() => this.loadVendeurs());
    }
  }
}
